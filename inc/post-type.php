...
function ced_taxonomy_select( $post_id, $taxonomy, $multiple = false ) {
	$terms = get_terms( $taxonomy, array( 'hide_empty' => false ) );
	$post_terms = wp_get_post_terms( $post_id, $taxonomy );
	$post_term_ids = wp_list_pluck( $post_terms, 'term_id'); 
	
	$html = sprintf( 
		'<select name="tax_input[%s][]" %s>', 
		$taxonomy, 
	...
	foreach( $terms as $term ) {
		$html .= sprintf( 
			...
			$term->term_id, 
			selected( in_array( $term->term_id, $post_term_ids ), true, false ), 
			$term->name 
		);	
	}
	$html .= '</select>';
...
function ced_select_taxonomy_box( $post, $meta_box ) {
	ced_taxonomy_select( $post->ID, $meta_box['args']['taxonomy'], false );
}

function ced_multiselect_taxonomy_box( $post, $meta_box ) {
	ced_taxonomy_select( $post->ID, $meta_box['args']['taxonomy'], true );
}